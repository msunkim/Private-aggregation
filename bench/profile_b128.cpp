...
// Pick a large-domain prime t ~ 2^60 with t = 1 (mod 2^17). The 128-bit native backend
// lifts the plaintext-modulus ceiling, so t ~ 2^60 holds a 60-bit root
// (<= 20-bit category || 40-bit tag): ~10^6 categories at lambda_sv = 2^-40.
static int64_t pickT() {
    NTL::ZZ Mmod  = NTL::conv<NTL::ZZ>((long)131072);
    NTL::ZZ start = ((NTL::conv<NTL::ZZ>((long)1) << 60) / Mmod) * Mmod + 1;
...
int main(int argc, char** argv) {
    ...
    std::cout << "t = " << t << " (~2^60, 128-bit native), security = 128-bit, trials = " << TRIALS
